...
int main(int argc, char const *argv[]) {
  ...
  printf("Writer %d: a writer is coming\n", getpid());
  ...
  if (rwt->waiting_table.writer || rwt->accessing_table.reader || rwt->accessing_table.writer) {
    ...
    printf("Writer %d: waiting             (waiting-reader:       %2d, waiting-writer: %2d -> %2d)\n", 
           getpid(), rwt->waiting_table.reader, rwt->waiting_table.writer-1, rwt->waiting_table.writer);

    // waiting for notification
    sem_wait(sem_writable);
    sem_wait(sem_table_mutex);
    rwt->waiting_table.writer--;
    printf("Writer %d: no need to wait     (waiting-reader:       %2d, waiting-writer: %2d -> %2d)\n", 
           getpid(), rwt->waiting_table.reader, rwt->waiting_table.writer+1, rwt->waiting_table.writer);
  }
  rwt->accessing_table.writer++;
  printf("Writer %d: accessing           [viewing-reader:       %2d, viewing-writer: %2d -> %2d]\n",
         getpid(), rwt->accessing_table.reader, rwt->accessing_table.writer-1, rwt->accessing_table.writer);
  sem_post(sem_table_mutex);
  ...
  sem_wait(sem_table_mutex);
  rwt->accessing_table.writer--;
  printf("Writer %d: leaving             [viewing-reader:       %2d, viewing-writer: %2d -> %2d]\n",
         getpid(), rwt->accessing_table.reader, rwt->accessing_table.writer+1, rwt->accessing_table.writer);
  if      (rwt->waiting_table.writer > 0) sem_post(sem_writable);
  else if (rwt->waiting_table.reader > 0) sem_post(sem_readable);
  sem_post(sem_table_mutex);
