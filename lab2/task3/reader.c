...
int main(int argc, char const *argv[]) {
  ...
  printf("Reader %d: a reader is coming\n", getpid());
  ...
  if (rwt->waiting_table.writer || rwt->accessing_table.writer) {
    ...
    printf("Reader %d: waiting             (waiting-reader: %2d -> %2d, waiting-writer:       %2d)\n",
           getpid(), rwt->waiting_table.reader-1, rwt->waiting_table.reader, rwt->waiting_table.writer);
    ...
    printf("Reader %d: no need to wait     (waiting-reader: %2d -> %2d, waiting-writer:       %2d)\n", 
           getpid(), rwt->waiting_table.reader+1, rwt->waiting_table.reader, rwt->waiting_table.writer);
  }
  rwt->accessing_table.reader++;
  printf("Reader %d: accessing           [viewing-reader: %2d -> %2d, viewing-writer:       %2d]\n",
         getpid(), rwt->accessing_table.reader-1, rwt->accessing_table.reader, rwt->accessing_table.writer);
  sem_post(sem_table_mutex);
  ...
  sem_wait(sem_table_mutex);
  rwt->accessing_table.reader--;
  printf("Reader %d: leaving             [viewing-reader: %2d -> %2d, viewing-writer:       %2d]\n",
         getpid(), rwt->accessing_table.reader+1, rwt->accessing_table.reader, rwt->accessing_table.writer);
